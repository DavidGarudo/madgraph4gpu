#WORKS FOR INTEL AS WELL (SPECIFY USE PYTHON 3.9 FOR INTEL)
 #Block for General Code
#Works for NVIDIA Confirmed
import re
import subprocess
gpu_info = {}
...
def getgpucardtype():
    try:
        ...
        # Execute the 'lspci' command with the appropriate arguments to list Intel GPUs
        intel_output = subprocess.check_output(['lspci', '-nnk', '-d', '8086:'], universal_newlines=True)

        # Search for Intel GPUs in the output
        intel_lines = [line for line in intel_output.split('\n') if 'VGA compatible controller' in line]

        # Extract the GPU card types from the Intel lines
...
def get_num_sm(vendor):
    if vendor == "amd":
        ...
        gpu_info["num_sm"] = "Not applicable"
    elif vendor == "nvidia":
